...
class MultiPidbTest {
  ...
  def testMultiDbs(): Unit = {
    ...
    val db2 = PidbConnector.openDatabase(new File("./testdb1"),
      new File("./neo4j.conf"));
    val db3 = PidbConnector.openDatabase(new File("./testdb2"),
      new File("./neo4j.conf"));

    testCreateBlob(db2, new File("./test.png"))
    testCreateBlob(db3, new File("./test1.png"))
    testQuery(db2, new File("./test.png"))
    testQuery(db3, new File("./test1.png"))
