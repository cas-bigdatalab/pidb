...
public class DynamicArrayStore extends AbstractDynamicStore
{
                                            ...
                                            DynamicRecordAllocator recordAllocator )
    {
        ...
        {
            Blob blob = array[i];
            byte[] bytes = BlobIO.of((TransactionRecordStateAware)recordAllocator).encodeBlob(blob);
    ...
    public static Value getRightArray( Pair<byte[],byte[]> data, Config conf )
    {
    ...
    {
        return getRightArray( readFullByteArray( records, PropertyType.ARRAY ) ,this.configuration).asObject();
    }
}
