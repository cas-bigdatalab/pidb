...
import org.neo4j.cypher.internal.runtime.interpreted.commands.{PathExtractorExpression, expressions => commandexpressions, predicates, values => commandvalues}
...
import org.neo4j.cypher.internal.v3_4.{expressions => ast, functions}
...
object CommunityExpressionConverter extends ExpressionConverter {
  ...
  private def regexMatch(e: ast.RegexMatch, self: ExpressionConverters) = self.toCommandExpression(e.rhs) match {
    case literal: commandexpressions.Literal =>
      LiteralRegularExpression(self.toCommandExpression(e.lhs), literal)
    case command =>
      RegularExpression(self.toCommandExpression(e.lhs), command)
  }
